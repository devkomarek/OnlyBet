...
/**
 * Created by komarekzm on 27/07/2016.
 * OnlyBet
 * *
 */
public class TipsDatabase {
    ...
    private TipsDatabaseHelper helper;
    private static TipsDatabase database;

    public static TipsDatabase getInstance(Context c) {
        if (database == null) {
            database = new TipsDatabase(c);
        }
        return database;
    }

    private TipsDatabase(Context c) {
        helper = new TipsDatabaseHelper(c);
    }
    ...
    private static class TipsDatabaseHelper extends SQLiteOpenHelper {
        ...
        TipsDatabaseHelper(Context context) {
